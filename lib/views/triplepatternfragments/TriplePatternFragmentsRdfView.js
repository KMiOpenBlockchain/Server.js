...
var dcTerms = 'http://purl.org/dc/terms/',
    ...
    blondie = 'http://www.semanticblockchain.com/Blondie.owl#',
    eth = 'http://ethon.consensys.net/';
...
TriplePatternFragmentsRdfView.prototype._generateRdf = function (settings, data, metadata, done) {
  var datasource = settings.datasource, fragment = settings.fragment, query = settings.query,
      results = settings.results, source = settings.sourceObj, metadataDone = false;
  ...
  results.getProperty('metadata', function (meta) {
    ...
    if (source.blockchainType && source.blockchainType === 'Ethereum') {
      metadata(fragment.pageUrl, rdf + 'type', prov + 'Entity');
      metadata(fragment.pageUrl, prov + 'qualifiedGeneration', '_:qualifiedGeneration');
      metadata('_:qualifiedGeneration', rdf + 'type', prov + 'Generation');
      metadata('_:qualifiedGeneration', prov + 'atTime', '"' + Date.now().toISOString() + '"^^' + xsd + 'dateTime');
      metadata('_:qualifiedGeneration', prov + 'atLocation', '_:provLocation');
      metadata('_:provLocation', rdf + 'type', blondie + 'ContractEthereumAccount');
      metadata('_:provLocation', blondie + 'codeHashContractEthereumAccount', '"' + source.dataAddress + '"^^' + xsd + 'string');
      metadata('_:provLocation', dcTerms + 'source', source.nodeHost + ':' + source.nodePort);
      metadata('_:provLocation', eth + 'ContractABI', '"' + JSON.stringify(source.ContractABI) + '"^^' + xsd + 'string');
      metadata(source.nodeHost + ':' + source.nodePort, rdf + 'type', eth + 'Blockchain');
    }
