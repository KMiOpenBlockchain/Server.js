...
Datasource.prototype.select = function (query, settings, onError) {
  if (!this.initialized)
  ...
  // Execute the query
  try { this._executeQuery(query, settings, destination); }
  catch (error) { outputTriples.emit('error', error); }
  return outputTriples;
};

// Writes the results of the query to the given destination
Datasource.prototype._executeQuery = function (query, settings, destination) {
